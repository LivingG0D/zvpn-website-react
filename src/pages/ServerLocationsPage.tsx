...
const features = [
	...
	},
	{
		...
		description:
			"All servers use military-grade encryption and security protocols",
	},
	{
		icon: Users,
		title: "Load Balancing",
		description:
			"Intelligent load balancing ensures optimal server performance",
	},
	{
		icon: Server,
...
export default function ServerLocationsPage() {
	...
	const totalServers = serverLocations.reduce(
		(total, region) =>
			total +
			region.countries.reduce(
				(regionTotal, country) => regionTotal + country.servers,
				0
			),
		0
	);

	const totalCountries = serverLocations.reduce(
		...
		<div className="min-h-screen py-16 px-4 bg-slate-900">
			...
			<div className="max-w-4xl mx-auto text-center mb-16">
				...
				<p className="text-xl text-muted-foreground max-w-2xl mx-auto">
					Connect to any of our {totalServers}+ servers across {totalCountries}{" "}
					countries for fast, secure, and reliable VPN connections.
				</p>
			</div>
					...
					<div className="flex gap-2 flex-wrap">
						...
						{serverLocations.map((region) => (
							<Button
								key={region.region}
								variant={
									selectedRegion === region.region ? "default" : "outline"
								}
				...
				<div className="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-4 gap-6">
					{features.map((feature, index) => {
						...
						return (
							<Card
								key={index}
								className="bg-card/30 backdrop-blur-sm border-border/50 hover:bg-card/60 transition-all duration-300 text-center"
							>
								<CardContent className="pt-6">
